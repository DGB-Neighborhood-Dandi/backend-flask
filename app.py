...
def gen_frames():
    while True:
        ...
        if not ret:
            ...
            results = model(frame)
            annotated_frame = results.render()[0]  # render()의 반환 값은 리스트이므로 첫 번째 요소 사용
            annotated_frame = cv2.cvtColor(annotated_frame, cv2.COLOR_RGB2BGR)  # OpenCV 형식으로 변환
            ...
            if detect_cell_phone:
                socket_io.emit('phone_detection', {'phone_detected': True}, namespace='/video')
            else:
                socket_io.emit('phone_detection', {'phone_detected': False}, namespace='/video')
